...
'''
        ...
        if not build_cmd:
            # try to find sip-build in Python's Scripts directories
            import sysconfig, site
            search_dirs = [
                sysconfig.get_path('scripts'),
                os.path.join(sysconfig.get_path('data'), 'Scripts'),
                os.path.join(get_path('platlib'), '..', 'Scripts'),
            ]
            # also check user site-packages Scripts
            user_base = site.getusersitepackages()
            if user_base:
                search_dirs.append(
                    os.path.join(os.path.dirname(user_base), 'Scripts'))
            for scripts_dir in search_dirs:
                for name in ('sip-build.exe', 'sip-build'):
