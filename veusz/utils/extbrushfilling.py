...
def brushExtFillPath(painter, extbrush, path, ignorehide=False,
    ...
    if style in ('linear gradient', 'radial gradient'):
        # gradient fill
        color1 = extbrush.get('color').color(painter, dataindex=dataindex)
        color2 = extbrush.get('color2').color(painter, dataindex=dataindex)
        if extbrush.transparency > 0:
            alpha = (100-extbrush.transparency) / 100.
            color1.setAlphaF(color1.alphaF() * alpha)
            color2.setAlphaF(color2.alphaF() * alpha)

        rect = path.boundingRect()
        if style == 'linear gradient':
            grad = qt.QLinearGradient(rect.topLeft(), rect.bottomLeft())
        else:
            grad = qt.QRadialGradient(rect.center(), max(rect.width(), rect.height()) * 0.5)
