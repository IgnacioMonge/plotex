...
"""ODS (.ods) import dialog tab."""

from .. import qtall as qt
from .. import utils
from ..dialogs import importdialog
from . import defn_ods

def _(text, disambiguation=None, context='Import'):
    return qt.QCoreApplication.translate(context, text, disambiguation)


# ── Thread-safe standalone reader (no UI access) ────────────────

def _read_ods_preview(filename, sheet_name='', skiprows=0, max_rows=20):
    """Read ODS file for preview. Thread-safe."""
    sheet_names, allrows = defn_ods._read_ods_file(filename, sheet_name)

    if skiprows > 0:
        allrows = allrows[skiprows:]

    rows = allrows[:max_rows]
    maxcols = max(len(r) for r in rows) if rows else 0
    return sheet_names, rows, maxcols


# ── Dialog tab ──────────────────────────────────────────────────

class ImportTabODS(importdialog.ImportTab):
    """Tab for importing ODS files."""

    resource = 'import_excel.ui'   # reuse same UI layout
    filetypes = ('.ods',)
    filefilter = _('OpenDocument spreadsheet files')
    ...
    def doPreview(self, filename, encoding):
        """Start async preview of ODS file."""
        ...
        self._preview_thread = importdialog.PreviewThread(
            _read_ods_preview, filename, sheet_name, skiprows)
    ...
    def doImport(self, doc, filename, linked, encoding, prefix, suffix,
        ...
        op = defn_ods.OperationDataImportODS(params)

        # Phase 1: heavy I/O in background thread with progress dialog
        progress = qt.QProgressDialog(
            _("Reading ODS file..."), None, 0, 0, self.dialog)
...
importdialog.registerImportTab(_('&ODS'), ImportTabODS)
