...
_simple_numeric_re = re.compile(r'^(\w+)\(numeric\)$')
...
def ImportString(comm, descriptor, dstring, useblocks=False):
    ...
    # fast path: single numeric dataset during loading
    # ImportString('name(numeric)', '''1.0\n2.0\n...''')
    if not useblocks and comm.document.loading:
        m = _simple_numeric_re.match(descriptor.strip())
        if m:
            name = m.group(1)
            try:
