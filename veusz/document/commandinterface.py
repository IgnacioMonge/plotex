...
class CommandInterface(qt.QObject):
    ...
    # map old setting names to new paths for backwards compatibility
    _setting_compat = {
    ...
    def Set(self, setting_path, val):
        ...
        # apply compatibility renames
        setting_path = self._setting_compat.get(setting_path, setting_path)
        try:
            setn = self.document.resolveSettingPath(
                self.currentwidget, setting_path)
        except (ValueError, KeyError):
            if self.document.loading:
                # silently skip unknown settings when loading old files
                return
            raise
        if self.document.loading:
            # fast path: set directly, skip Operation overhead
            setn.set(val)
        else:
    ...
    def SetToReference(self, setting_path, val):
        ...
        if self.document.loading:
            setn.set(setting.Reference(val))
        else:
    ...
    def SetDataExpression(self, name, val, symerr=None, negerr=None, poserr=None,
        ...
        if self.verbose:
            print(
                _(
                    "Set dataset '%s' based on expression:\n"
                    " Values = %s\n"
                    " Symmetric errors = %s\n"
                    " Negative errors = %s\n"
                    " Positive errors = %s"
    ...
    def SetData2DExpression(self, name, expr, linked=False):
        ...
        if self.verbose:
            print(
                _(
                    "Set 2D dataset '%s' based on expressions\n"
                    " expression = %s\n"
    ...
    def SetData2DExpressionXYZ(self, name, xexpr, yexpr, zexpr, linked=False):
        ...
        if self.verbose:
            print(
                _(
                    "Made 2D dataset '%s' based on expressions:\n"
                    " X expression = %s\n"
                    " Y expression = %s\n"
                    " Z expression = %s\n"
    ...
    def SetData2DXYFunc(self, name, xstep, ystep, expr, linked=False):
        ...
        if self.verbose:
            print(
                _(
                    "Set 2D dataset '%s' based on function of x and y\n"
                    " X steps = %s\n"
                    " Y steps = %s\n"
                    " Expression = %s\n"
                    " linked to expression = %s\n"
