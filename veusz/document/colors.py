...
colorthemes = {
    ...
    # Okabe-Ito (2008) — universal colorblind-safe palette
    ...
    # Paul Tol's qualitative palette (vibrant)
    ...
    # Paul Tol's muted palette — good for fills and areas
    'tol-muted': [
        '#CC6677',  # rose
        '#332288',  # indigo
        '#DDCC77',  # sand
    ...
    # Tableau 10 — widely used in data visualization
    'tableau10': [
        '#4E79A7',  # blue
        '#F28E2B',  # orange
        '#E15759',  # red
    ...
    # Plotex — custom perceptually balanced palette for scientific plots
    # Designed for: print-safe, screen-safe, colorblind-accessible
    'plotex': [
        '#3366CC',  # strong blue
        '#DC3912',  # vivid red
        '#109618',  # green
...
colorthemes['default-latest'] = colorthemes['plotex']
...
class Colors(qt.QObject):
    ...
    def __init__(self):
        ...
        self.maxthemeidx = 1
        # current theme (default1 = ColorBrewer sets for auto-coloring)
        self.colortheme = 'default1'
...
class ColorModel(qt.QAbstractListModel):
    ...
    def __init__(self, parent, colors):
        ...
        # cache of icons for colors indexed by rgba value (limited size)
        self._iconcache_maxsize = 256
        self.iconcache = {}
    ...
    def data(self, index, role):
        ...
        if role == qt.Qt.ItemDataRole.DisplayRole or role == qt.Qt.ItemDataRole.EditRole:
            ...
            if rgba not in self.iconcache:
                if len(self.iconcache) >= self._iconcache_maxsize:
                    # evict oldest entries
                    keys = list(self.iconcache.keys())
                    for k in keys[:len(keys)//2]:
                        del self.iconcache[k]
                self.makeIcon(color)
