...
class OperationSettingPropagate(Operation):
    ...
    def __init__(self, setting, widgetname = None, root = None,
        ...
        # use the resolved value, not a Reference
        self.val = setting.get()
    ...
    @staticmethod
    def _iterWidgets(root, name, typename, maxlevels):
        """Yield widgets matching name/typename criteria.

        If name or typename are None, ignore that criterion.
        maxlevels is the maximum depth to search (<0 = infinite).
        Uses iterative stack instead of recursion.
        """
        stack = [(root, maxlevels)]
        while stack:
...
class OperationWidgetsDelete(Operation):
    ...
    def do(self, document):
        ...
        for path in widgetpaths:
            ...
            oldparent.removeChild(widget.name)
                ...
                for c in parent.children:
                    if c.typename == 'axis' and c.name == axis_name:
                        ...
                        if lbl.isDefault():
                            ...
                            if lbl.isReference():
                                self._saved_labels[lbl.path] = lbl.getReference()
                            else:
                                self._saved_labels[lbl.path] = lbl.val
                            lbl.set(label_text)

        return w

    def undo(self, document):
        """Remove the added widget and restore axis labels."""

        # Restore axis labels (preserves reference/default state)
        for path, old in self._saved_labels.items():
            try:
                setn = document.resolveSettingPath(None, path)
                setn.set(old)
...
class OperationDatasetSet(Operation):
    ...
    def do(self, document):
        ...
        document._setDataUnlocked(self.datasetname, self.dataset)

    def undo(self, document):
        """Undo the data setting."""

        if self.olddata is None:
            document._deleteDataUnlocked(self.datasetname)
        else:
...
class OperationDatasetDuplicate(Operation):
    ...
    def do(self, document):
        ...
        document._setDataUnlocked(self.duplname, duplicate)

    def undo(self, document):
        """Delete the duplicate"""

        if self.olddata is None:
            document._deleteDataUnlocked(self.duplname)
        else:
...
class OperationApplyThemeToWidget(Operation):
    ...
    def __init__(self, widget, theme_settings):
        ...
        theme_settings: dict like plotthemes THEMES[key]['settings'],
            e.g. {'graph/Background/color': 'white', 'axis/Line/color': ...}
        """
        self.widgetpath = widget.path
    ...
    def do(self, document):
        ...
        # Build flat list of all widgets in subtree
        allwidgets = []
        widget.buildFlatWidgetList(allwidgets)

        # Index widgets by typename for fast lookup
        widgets_by_type = {}
        for w in allwidgets:
            widgets_by_type.setdefault(w.typename, []).append(w)
        ...
        for theme_path, value in self.theme_settings.items():
            ...
            for w in targets:
                ...
                # Save for undo
                if setn.isReference():
                    self.restorevals[setn.path] = setn.getReference()
                else:
                    self.restorevals[setn.path] = setn.get()

                # Set as concrete value (unlinked from stylesheet)
                setn.set(value)

        # Top-level theme keys like 'Font/size', 'Line/color' etc. are
        # stylesheet-global and cannot be applied per-widget.  Skip them
        # to avoid a supposedly local operation leaking into global state.

    def undo(self, document):
        for setpath, setval in self.restorevals.items():
            try:
                s = document.resolveSettingPath(None, setpath)
                s.set(setval)
            except (ValueError, KeyError):
                pass
