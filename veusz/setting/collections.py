...
class Line(Settings):
    ...
    def makeQPen(self, painter):
        ...
        cached = getattr(self, '_qpen_cache', None)
        if cached is not None and cached[0] == cache_key:
            return qt.QPen(cached[1])
        ...
        self._qpen_cache = (cache_key, pen)
        return qt.QPen(pen)
...
class Brush(Settings):
    ...
    def makeQBrush(self, painter):
        ...
        cached = getattr(self, '_qbrush_cache', None)
        if cached is not None and cached[0] == cache_key:
            return qt.QBrush(cached[1])

        color = self.get('color').color(painter)
        if self.transparency > 0:
            color.setAlphaF((100-self.transparency) / 100.)
        brush = qt.QBrush(color, self.get('style').qtStyle())

        self._qbrush_cache = (cache_key, brush)
        return qt.QBrush(brush)
...
class BrushExtended(Settings):
    ...
    def __init__(self, name, **args):
        ...
        self.add( setting.Color(
            'color2', 'background',
            descr=_('Second colour for gradient fills'),
            usertext=_('Color 2')) )
...
class PointLabel(Text):
    ...
    def __init__(self, name, **args):
        ...
        self.add( setting.Bool(
            'avoidOverlap', True,
            descr=_('Automatically adjust label positions to reduce overlap'),
            usertext=_('Avoid overlap'),
            formatting=True), 0 )
        self.add( setting.Float(
            'angle', 0.,
...
class DataColor(Settings):
    """Settings for coloring points using data values."""

    def __init__(self, name, dimensions=1):
        Settings.__init__(self, name, setnsmode='groupedsetting',
                          usertext=_('Color by data'),
                          descr=_('Color each point by a numeric dataset '
                                  '(requires Marker fill → Color map ≠ none)'))
        self.add( setting.DatasetExtended(
            'points', '',
            descr=_(
                'Dataset whose values determine marker color '
                '(mapped through the Color map in Marker fill)'),
            dimensions=dimensions,
            usertext=_('Dataset')) )
        self.add( setting.Float(
            'min', 0.,
            descr=_('Value mapped to the start of the color map'),
            usertext=_('Min val') ))
        self.add( setting.Float(
            'max', 1.,
            descr=_('Value mapped to the end of the color map'),
            usertext=_('Max val') ))
        self.add( setting.Choice(
            'scaling',
            ['linear', 'sqrt', 'log', 'squared'],
            'linear',
            descr=_('Scaling applied before mapping to color'),
            usertext=_('Scaling')))
