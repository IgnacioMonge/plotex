...
def evalDatasetExpression(doc, origexpr, datatype='numeric',
    ...
    # set up environment for evaluation (lightweight overlay on shared context)
...
class DatasetExpression(Dataset1DBase):
    ...
    def _evaluatePart(self, expr, part):
        ...
        # set up environment to evaluate expressions in (lightweight overlay)
        extra = {'_DS_': self.evaluateDataset}
        ...
        try:
            ...
            evalout = N.asarray(result, dtype=N.float64)
        ...
        if part != 'data':
            ...
            if evalout.shape == ():
                ...
                evalout = N.full(data.shape, evalout.item(), dtype=N.float64)
            else:
                # 1-dimensional - make it right size and trim
                target = N.full(data.shape, N.nan, dtype=N.float64)
                n = min(evalout.shape[0], target.shape[0])
                target[:n] = evalout[:n]
                evalout = target
        else:
            if evalout.shape == ():
                # zero dimensional - make a single point
                evalout = evalout.reshape(1)
...
class Dataset2DXYZExpression(Dataset2DBase):
    ...
    def evalDataset(self):
        ...
        environment = {**self.document.evaluate.context, '_DS_': self.evaluateDataset}
