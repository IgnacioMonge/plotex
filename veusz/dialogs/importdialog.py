...
class ImportTab(qt.QWidget):
    ...
    # set to True in subclasses that handle their own suspend/cursor
    handles_own_import_wrapper = False
...
class PreviewThread(qt.QThread):
    """Generic background thread for reading file data for preview.

    Usage:
        thread = PreviewThread(read_func, arg1, arg2, ...)
        thread.sigResult.connect(on_done)
        thread.start()
...
class ImportDialog(VeuszDialog):
    ...
    def showEvent(self, event):
        """Clear filename on show so it doesn't auto-preview the last file."""
