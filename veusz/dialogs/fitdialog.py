...
class FitDialog(qt.QDialog):
    ...
    def _fitwidgetAlive(self):
        """Return True if the fit widget is still attached to a document.

        Users can delete the widget from the tree while the dialog is open;
        the document drops it but our reference may still exist.  Methods
        that read .fitwidget.settings crash with RuntimeError otherwise.
        """
        fw = self.fitwidget
        if fw is None:
            return False
        try:
            # accessing .document detects detached/deleted widgets
            return fw.document is not None and fw.parent is not None
    ...
    def _setParamRow(self, row, name, initial, fitted="", error=""):
        ...
        nameItem.setFlags(nameItem.flags() & ~qt.Qt.ItemFlag.ItemIsEditable)
        ...
        fittedItem.setFlags(fittedItem.flags() & ~qt.Qt.ItemFlag.ItemIsEditable)
        ...
        errItem.setFlags(errItem.flags() & ~qt.Qt.ItemFlag.ItemIsEditable)
    ...
    def _saveToWidget(self):
        ...
        for attr, edit in [
            ...
            if getattr(s, attr) != edit:
                ops.append(document.OperationSettingSet(s.get(attr), edit))

        var = self.varCombo.currentText()
        if s.variable != var:
            ops.append(document.OperationSettingSet(s.get("variable"), var))

        params = self._getParams()
        if params != s.values:
            ops.append(document.OperationSettingSet(s.get("values"), params))

        defErr = self.defErrSpin.value()
        if defErr != s.defErr:
            ops.append(document.OperationSettingSet(s.get("defErr"), defErr))
        ...
        for attr, check in [
            ...
            if check.isChecked() != getattr(s, attr):
                ops.append(document.OperationSettingSet(s.get(attr), check.isChecked()))

        if ops:
            doc.applyOperation(
                document.OperationMultiple(ops, descr=_("update fit settings"))
            )

    def _doFit(self):
        """Run the fit and update plot in real time.

        Uses a single bounded event-loop pump (UI-input-events excluded)
        to refresh the "Fitting…" label, then runs the fit synchronously
        and lets the document's own signalModified trigger the repaint
        afterwards. Previously this routine called ``processEvents()``
        four times — including UI input events — which let the user
        click ``Close`` (firing ``accept()``) while ``fitwidget.actionFit``
        was still running, leading to ``_addResultLabel`` operating on a
        destroyed dialog. The ``_fit_in_progress`` flag and the busy
        cursor below also block recursive ``_doFit`` if a queued click
        slips through despite the disabled button.
        """
        ...
        # Single UI refresh, input events excluded so the user can't
        # close/cancel mid-fit (which would tear down ``self``).
        qt.QApplication.processEvents(
            qt.QEventLoop.ProcessEventsFlag.ExcludeUserInputEvents
        )
            ...
            if s.chi2 > 0:
                ...
                for row in range(self.paramTable.rowCount()):
                    ...
                    if name in s.paramErrors and s.paramErrors[name] > 0:
                        ...
                        if errItem:
                            errItem.setText("\u00b1 %.4g" % s.paramErrors[name])
    ...
    def _addResultLabel(self, s):
        ...
        try:
            ...
            if data and data[0] is not None and data[1] is not None:
                ...
                compiled = doc.evaluate.compileCheckedExpression(s.function)
                evalenv[s.variable] = xd[mask]
                evalenv.update(values)
                yfit = eval(compiled, evalenv) + xd[mask] * 0.0
        ...
        if "params" in fmt:
            for name in sorted(values.keys()):
                val = values[name]
                if name in errors and errors[name] > 0:
                    parts.append("%s = %.4g \\pm %.4g" % (name, val, errors[name]))
                else:
        ...
        if label is None:
            ...
            doc.applyOperation(
                document.OperationWidgetAdd(
                    graph, "label", autoadd=False, name=label_name
                )
            )
            label = graph.getChild(label_name)
            if label is None:
            ...
            ops = [
                document.OperationSettingSet(label.settings.get("xPos"), [0.05]),
                document.OperationSettingSet(label.settings.get("yPos"), [0.95]),
                document.OperationSettingSet(label.settings.get("alignVert"), "top"),
                document.OperationSettingSet(label.settings.get("alignHorz"), "left"),
            ]
            doc.applyOperation(
                document.OperationMultiple(ops, descr=_("position fit label"))
            )

        # always update the text
        doc.applyOperation(
            document.OperationSettingSet(label.settings.get("label"), label_text)
        )
