...
class PreferencesDialog(qt.QDialog):
    ...
    def __init__(self, mainwindow):
        ...
        # Snapshot of every setting/state we modify with live preview
        # (color scheme, app font). Used by reject() so that Cancel truly
        # cancels — previously color_scheme and the application font were
        # mutated as the user moved combos/spinners and never reverted on
        # Cancel, making the button a misleading no-op.
        app = qt.QApplication.instance()
        self._origState = {
            "color_scheme": self.setdb.get("color_scheme", None),
            "ui_font": self.setdb.get("ui_font", ""),
            "ui_font_size": self.setdb.get("ui_font_size", 9),
            "ui_font_bold": self.setdb.get("ui_font_bold", False),
            "ui_font_italic": self.setdb.get("ui_font_italic", False),
            "app_font": qt.QFont(app.font()) if app is not None else None,
        }
    ...
    def _buildAppearancePage(self):
        ...
        # connect live preview for font changes (debounced via singleShot
        # timer so spin-arrow drags and rapid toggles coalesce into one
        # app-wide setFont() call rather than firing four times a second)
        self._fontLiveTimer = qt.QTimer(self)
        self._fontLiveTimer.setSingleShot(True)
        self._fontLiveTimer.setInterval(120)
        self._fontLiveTimer.timeout.connect(self._applyFontLive)
        trigger = lambda *a: self._fontLiveTimer.start()
        self.fontCombo.currentFontChanged.connect(trigger)
        self.fontSizeSpin.valueChanged.connect(trigger)
        self.fontBoldCheck.toggled.connect(trigger)
        self.fontItalicCheck.toggled.connect(trigger)
    ...
    def _buildDataPage(self):
        ...
        (
            self.dirDocCWDRadio
            if self.setdb["dirname_usecwd"]
            else self.dirDocPrevRadio
        ).setChecked(True)
        layout.addWidget(self.dirDocPrevRadio)
        layout.addWidget(self.dirDocCWDRadio)

        self.docFileAddImportPaths = qt.QCheckBox(
            _("Add document path to import paths")
        )
    ...
    def reject(self):
        """Cancel button: revert any live-preview mutations.

        Color-scheme combo and font widgets apply globally as the user
        interacts with them, so without this override Cancel keeps the
        previewed state — the button label lies. Restore both the
        settingdb keys and the in-process QApplication font/scheme so
        the UI snaps back exactly to where the dialog opened.
        """
        orig = getattr(self, "_origState", None)
        if orig is not None:
            app = qt.QApplication.instance()
            try:
                if orig.get("color_scheme") is not None:
                    self.setdb["color_scheme"] = orig["color_scheme"]
                    if app is not None and hasattr(app, "applyColorScheme"):
                        app.applyColorScheme(orig["color_scheme"])
            except Exception:
                pass
            try:
                self.setdb["ui_font"] = orig["ui_font"]
                self.setdb["ui_font_size"] = orig["ui_font_size"]
                self.setdb["ui_font_bold"] = orig["ui_font_bold"]
                self.setdb["ui_font_italic"] = orig["ui_font_italic"]
                if app is not None and orig.get("app_font") is not None:
                    app.setFont(orig["app_font"])
            except Exception:
                pass
    ...
    def accept(self):
        ...
        setdb["plot_updatepolicy"] = self.plotwindow.updateintervals[
            self.intervalCombo.currentIndex()
        ][0]
