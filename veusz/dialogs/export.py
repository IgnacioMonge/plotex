...
def _makeSection(text):
    ...
    lbl.setStyleSheet(
        ...
        'color: palette(text); margin-top: 6px;')
    return lbl
...
class ExportDialog(qt.QDialog):
    ...
    def _buildUI(self):
        ...
        self.editFileName.setToolTip(
            _('Use %PAGE%, %PAGE00%, %PAGE000%, %PAGENAME% '
              'for multi-file export'))
        ...
        vector_fmts = [('pdf', 'PDF'), ('eps', 'EPS'), ('ps', 'PS'),
                       ('svg', 'SVG'), ('emf', 'EMF')]
        bitmap_fmts = [('png', 'PNG'), ('bmp', 'BMP'), ('jpg', 'JPG'),
                       ('tiff', 'TIFF'), ('xpm', 'XPM'), ('webp', 'WebP')]
        ...
        valre = qt.QRegularExpression(
            r'^[0-9]+(\s*-\s*[0-9]+)?'
            r'(\s*,\s*[0-9]+(\s*-\s*[0-9]+)?)*$')
        self.editPagePages.setValidator(
            qt.QRegularExpressionValidator(valre, self))
        pagesrow.addWidget(self.radioPagePages)
        pagesrow.addWidget(self.editPagePages)
        pagesrow.addStretch(1)

        self.checkMultiPage = qt.QCheckBox(
            _('Multiple pages in one file'))
        ...
        self.rowBackground = _makeRow(
            _('Background:'), self.exportBackgroundButton)
        ...
        self.checkOverwrite = qt.QCheckBox(
            _('Overwrite without asking'))
        ...
        self.btnClipboard.setToolTip(
            _('Copy current page to clipboard as image'))
    ...
    def _connectSignals(self):
        ...
        self.btnPublication.clicked.connect(
            lambda: self._applyPreset('publication'))
        self.btnWeb.clicked.connect(
            lambda: self._applyPreset('web'))
        self.btnPresentation.clicked.connect(
            lambda: self._applyPreset('presentation'))

        for fmt, btn in self.fmtButtons.items():
            btn.clicked.connect(lambda checked, f=fmt: self._formatClicked(f))

        self.radioPageSingle.clicked.connect(
            lambda: self._pageClicked('single'))
        self.radioPageAll.clicked.connect(
            lambda: self._pageClicked('all'))
        self.radioPagePages.clicked.connect(
            lambda: self._pageClicked('pages'))

        self.checkMultiPage.clicked.connect(self._updateSingleMulti)
        self.exportBackgroundButton.clicked.connect(
            self._backgroundClicked)
    ...
    def _updateSingleMulti(self):
        ...
        newfilename = os.path.join(
            self.dirname,
            templ.replace('%DOCNAME%', self.docname) +
            '.' + self.formatselected)
    ...
    def _updatePreview(self):
        ...
        try:
            ...
            maxdim = 190
            scale = min(maxdim / max(size[0], 1),
                        maxdim / max(size[1], 1), 1.0)
            w = max(int(size[0] * scale), 1)
            h = max(int(size[1] * scale), 1)

            # use PaintHelper like the render thread does
            helper = document.PaintHelper(
                self.document, (w, h), scaling=scale, dpi=dpi)
            self.document.paintTo(helper, pagenum)

            img = qt.QImage(
                w, h, qt.QImage.Format.Format_ARGB32_Premultiplied)
            img.fill(qt.QColor(255, 255, 255))

            painter = qt.QPainter(img)
            painter.setRenderHint(
                qt.QPainter.RenderHint.Antialiasing, True)
            painter.setRenderHint(
                qt.QPainter.RenderHint.TextAntialiasing, True)
    ...
    def _updateDimensions(self):
        ...
        try:
            ...
            size = self.document.pageSize(
                pagenum, dpi=(dpi, dpi), integer=True)
    ...
    def _backgroundClicked(self):
        ...
        color = setting.controls._getColor(
            qcolor, self, _("Choose color"), alpha=True)
        if color.isValid():
            self._updateExportBackground(
                utils.extendedColorFromQColor(color))
    ...
    def _showMessage(self, text):
        ...
        qt.QTimer.singleShot(4000, self.labelStatus.clear)
    ...
    def _copyToClipboard(self):
        ...
        try:
            ...
            dpi = (dpi_val, dpi_val)
            size = self.document.pageSize(
                pagenum, dpi=dpi, integer=True)

            helper = document.PaintHelper(
                self.document, size, dpi=dpi)
            self.document.paintTo(helper, pagenum)
    ...
    def _doExport(self):
        ...
        def checkDone():
            ...
            try:
                ...
                if pagecount[0] > 0:
                    self._showMessage(
                        _('Exported %d page(s)') % pagecount[0])
