...
class DatasetRelationModel(TreeModel):
    ...
    def setDocument(self, newdoc):
        ...
        try:
            self.doc.signalModified.disconnect(self.refresh)
        except (TypeError, RuntimeError):
            pass
        self.doc = newdoc
        newdoc.signalModified.connect(self.refresh)
        self.refresh()
    ...
    def makeGrpTree(self, coltitles, colitems, grouper, GrpNodeClass):
        ...
        grpnodes = {}
        for name, ds in list(self.doc.data.items()):
...
class DatasetsNavigatorTree(qt.QTreeView):
    ...
    def setDocument(self, newdoc):
        ...
        if self.mainwindow is not None:
            try:
                self.mainwindow.documentOpened.disconnect(self.expandAll)
            except TypeError:
                pass
            self.mainwindow.documentOpened.connect(self.expandAll)

        # disconnect old signals before reconnecting (avoid duplicates)
        try:
            ...
            pass
        try:
            self.model.rowsInserted.disconnect(self.slotNewRow)
        except TypeError:
            pass
    ...
    def selectDatasets(self, dsnames):
        """Select multiple datasets in the tree."""
        if not dsnames:
            return
        if len(dsnames) == 1:
            self.selectDataset(dsnames[0])
            return
    ...
    def getMenuUseAs(self, menu, dataset):
        ...
        Groups entries by page/graph for readability instead of
        showing a flat list of hundreds of paths.
        """

        dsname = self.doc.datasetName(dataset)
        groups = {}
        ...
        if not groups:
            menu.addAction(_('(no compatible settings)')).setEnabled(False)
            return
        ...
        for group_name in sorted(groups.keys()):
            ...
            for path in sorted(paths):
                # show short label: just the widget/setting part
                parts = path.strip('/').split('/')
                short = '/'.join(parts[2:]) if len(parts) > 2 else path
                def _setds(p=path):
                    self.doc.applyOperation(
                        document.OperationSettingSet(p, dsname))
                submenu.addAction(short, _setds)
...
class DatasetBrowser(qt.QWidget):
    ...
    def selectDatasets(self, dsnames):
        """Select multiple datasets in the tree."""
        self.navtree.selectDatasets(dsnames)

    def setDocument(self, newdoc):
        """Switch to a different document."""
        self.navtree.setDocument(newdoc)
