...
"""
Plotex interface which listens to stdin, and receives commands.
...
class InputListener(qt.QObject):
    ...
    def enablePickle(self, on=True):
        """Enable/disable JSON command mode for commands to/from veusz.

        Kept as 'Pickle' command name for backward compatibility,
        but now uses safe JSON serialization internally.
        """
        self.pickle = on
    ...
    def processLine(self, line):
        ...
        if self.pickle:
            # Safe JSON-based command processing
            # Expected format: JSON-encoded [name, args, namedargs]
            line = line.strip()
            try:
                name, args, namedargs = json.loads(line)
            except (json.JSONDecodeError, ValueError) as e:
                result = json.dumps({
                    '__exception__': True,
                    'type': 'ValueError',
                    'message': 'Invalid JSON command: %s' % str(e),
                })
            ...
            try:
                retn = self.ci.runCommand(name, args, namedargs)
