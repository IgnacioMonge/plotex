...
class BarPlotter(GenericPlotter):
    ...
    def getAxisLabels(self, direction):
        ...
        if s.direction != direction:
            ...
            # for grouped bars with labels per bar (not per group),
            # use individual bar positions
            if (labels is not None and len(labels) > len(p)
                    and s.mode == 'grouped'):
                posns = self.getPosns()
                if posns is not None and len(posns) == len(labels):
                    p = posns
        ...
        if ((s.direction == 'horizontal' and depname == 'sx') or
            ...
            if s.posn:
                ...
                if data:
                    ...
                    numds = len(data)
                    # for grouped bars, expand range so each bar
                    # has enough room for its label
                    if s.mode == 'grouped' and numds > 1 and maxlen == 1:
                        # N datasets × 1 value: expand range as if
                        # bars were at positions 1..N
                        axrange[0] = min(0.5, axrange[0])
                        axrange[1] = max(numds + 0.5, axrange[1])
                    else:
    ...
    def findBarPositions(self, lengths, positions, axes, posn):
        ...
        numds = len(lengths)
        maxlen = max(len(d.data) for d in lengths)

        if positions is None:
            # special case: N datasets × 1 value in grouped mode
            # → treat each dataset as its own category at 1, 2, ..., N
            if s.mode == 'grouped' and numds > 1 and maxlen == 1:
                p = N.arange(numds, dtype=float) + 1.0
            else:
                p = N.arange(maxlen) + 1.0
        else:
            p = positions.data
    ...
    def getPosns(self):
        """Return data-coordinate centre positions for each individual bar.

        For grouped bars with N datasets of 1 value each, returns
        positions 1, 2, ..., N (one per bar, as separate categories).
        For grouped bars with M>1 values, returns sub-positions within
        each group.  For stacked bars, returns one position per group.
        """
        s = self.settings
        doc = self.document
        lengths = s.get('lengths').getData(doc)
        ...
        positions = s.get('posn').getData(doc) if s.posn else None
        numds = len(lengths)
        maxlen = max(len(d.data) for d in lengths)
        ...
        if positions is None:
            ...
            p = N.array(positions.data, dtype=float)

        if s.mode == 'grouped' and numds > 1:
            if maxlen == 1:
                # N datasets × 1 value: separate positions
                # (matches findBarPositions special case)
                return N.arange(numds, dtype=float) + 1.0
            else:
                groupwidth = 1.0
                usablewidth = groupwidth * s.groupfill
                bardelta = usablewidth / numds
                all_posns = []
                for dsnum in range(numds):
                    centre = -usablewidth*0.5 + bardelta*dsnum + bardelta*0.5
                    all_posns.extend((p + centre).tolist())
                return N.array(sorted(all_posns))
        else:
            return p

    def getBarPlotterEdges(self, barindex, axes, widgetposn):
        """Return (left_px, right_px) plotter-coord edges of bar `barindex`.

        barindex is 0-based across all individual bars (for grouped:
        dataset 0 bar 0, dataset 1 bar 0, etc.).
        ...
        if s.mode == 'grouped' and numds > 1:
            ...
            ngroups = len(posns)
            dsnum = barindex // ngroups
            gidx = barindex % ngroups
            if dsnum >= numds or gidx >= ngroups:
                return None

            centre = posns[gidx] + (
                -usablewidth*0.5 + bardelta*dsnum +
                (bardelta - barwidth)*0.5 + barwidth*0.5)
            return (centre - barwidth*0.5, centre + barwidth*0.5)
        else:
            barwidth = maxwidth * s.barfill
            if barindex >= len(posns):
                return None
            centre = posns[barindex]
            return (centre - barwidth*0.5, centre + barwidth*0.5)
    ...
    def calcStackedPoints(self, dsvals, axis, widgetposn):
        ...
        for dsnum, data in enumerate(dsvals):
            ...
            lastneg = N.minimum(lastneg, new)
            lastpos = N.maximum(lastpos, new)
    ...
    def dataDraw(self, painter, axes, widgetposn, clip):
        ...
        # special case: N datasets × 1 value in grouped mode
        # → draw each bar separately at its own position with its own color
        numds = len(lengths)
        maxlen = max(len(d.data) for d in lengths)
        if s.mode == 'grouped' and numds > 1 and maxlen == 1:
            ishorz = s.direction == 'horizontal'
            vaxis = axes[not ishorz]
            barwidth = maxwidth * s.barfill
            zeropt = vaxis.dataToPlotterCoords(widgetposn, N.array([0.]))

            for dsnum in range(numds):
                dv = dsvals[dsnum]
                coord = vaxis.dataToPlotterCoords(
                    widgetposn, dv['data'][:1])
                p1 = barposns[dsnum:dsnum+1] - barwidth * 0.5
                p2 = barposns[dsnum:dsnum+1] + barwidth * 0.5
                if ishorz:
                    p = (zeropt, p1, coord, p2)
                else:
                    p = (p1, zeropt, p2, coord)
                self.plotBars(painter, s, dsnum, clip, p)

                # draw error bars
                self.drawErrorBars(
                    painter, barposns[dsnum:dsnum+1], barwidth,
                    dv['data'][:1], dv,
                    axes, widgetposn)
        else:
            fn = {
