...
from .axis import MajorTick, MinorTick, GridLine, MinorGridLine, AxisLabel, TickLabel
...
class Ternary(NonOrthGraph):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.Float(
                "fracsize", 1.0, descr=_("Fractional size of plot"), usertext=_("Size")
            )
        )

        s.add(
            setting.Bool(
                "reverse", False, descr=_("Reverse axes"), usertext=_("Reverse")
            )
        )
    ...
    def graphToPlotCoords(self, coorda, coordb):
        ...
        clist = [coordan, coordbn, 1.0 - coordan - coordbn]
    ...
    def drawFillPts(self, painter, brushext, cliprect, ptsx, ptsy):
        ...
        if filltype == "left":
            # project endpoints horizontally to the left edge of the triangle
            # left edge at height y: x = box[0] + (box[3] - y) * tan30
            xleft_end = self._box[0] + (self._box[3] - ptsy[-1]) * tan30
            xleft_start = self._box[0] + (self._box[3] - ptsy[0]) * tan30
            pts.append(qt.QPointF(xleft_end, ptsy[-1]))
            pts.append(qt.QPointF(xleft_start, ptsy[0]))
        elif filltype == "right":
            # project endpoints horizontally to the right edge of the triangle
            # right edge at height y: x = box[2] - (box[3] - y) * tan30
            xright_end = self._box[2] - (self._box[3] - ptsy[-1]) * tan30
            xright_start = self._box[2] - (self._box[3] - ptsy[0]) * tan30
            pts.append(qt.QPointF(xright_end, ptsy[-1]))
            pts.append(qt.QPointF(xright_start, ptsy[0]))
        elif filltype == "bottom":
            # project endpoints straight down to the bottom edge
            pts.append(qt.QPointF(ptsx[-1], self._box[3]))
            pts.append(qt.QPointF(ptsx[0], self._box[3]))
    ...
    def drawGraph(self, painter, bounds, datarange, outerbounds=None):
        ...
        d60 = 60.0 / 180.0 * math.pi
        ang = math.atan2(yw, xw / 2.0)
        if ang > d60:
            # taller than wider
            widthh = xw / 2
        ...
        self._size = max(min(s.fracsize, 1.0), 0.0)

        # make sure we don't go past the ends of the allowed range
        # value of origin of left axis at top
        self._orgleft = min(s.originleft, 1.0 - self._size)
        # value of origin of bottom axis at left
        self._orgbot = min(s.originbottom, 1.0 - self._size)
        # origin of right axis at bottom
        self._orgright = 1.0 - self._orgleft - (self._orgbot + self._size)
    ...
    def _drawTickSet(
        ...
        # left ticks
        x1 = (
            -(tickleft - self._orgleft) / self._size * self._width * sin30
            + (self._box[0] + self._box[2]) * 0.5
        )
        y1 = (tickleft - self._orgleft) / self._size * self._width * cos30 + self._box[
            1
        ]
        if reverse:
            x2 = x1 - tl
        ...
        # right ticks
        x1 = (
            -(tickright - self._orgright) / self._size * self._width * sin30
            + self._box[2]
        )
        y1 = (
            -(tickright - self._orgright) / self._size * self._width * cos30
            + self._box[3]
        )
        if reverse:
            x2 = x1 + tl * sin30
        ...
        if tickLabelSetn is not None and not tickLabelSetn.hide:
            ...
            for l, x, y in zip(
                tleftlabels, tickleftline[2] - off - sp, tickleftline[3]
            ):
                r = utils.Renderer(painter, font, x, y, l, 1, 0, 0, doc=self.document)
                bounds = r.render()
                wlableft = max(wlableft, bounds[2] - bounds[0])
            for l, x, y in zip(
                trightlabels, tickrightline[2] + off + sp, tickrightline[3]
            ):
                r = utils.Renderer(painter, font, x, y, l, -1, 0, 0, doc=self.document)
                bounds = r.render()
        ...
        if labelSetn is not None and not labelSetn.hide:
            ...
            # alignment
            align = {"at-minimum": -1, "centre": 0, "at-maximum": 1}[labelSetn.position]
            aoffset = {
                "at-minimum": -self._width / 2,
                "centre": 0,
                "at-maximum": self._width / 2,
            }[labelSetn.position]
            ...
            with utils.painter_state(painter):
                painter.translate(
                    self._box[0] + self._width / 2, self._box[3] + bdelta + off
                )
