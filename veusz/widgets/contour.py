...
def _catmullRomPath(poly):
    """Convert a QPolygonF to a smooth Catmull-Rom spline QPainterPath."""
    path = qt.QPainterPath()
    n = poly.count()
    if n < 2:
        return path
    pts = [poly.at(i) for i in range(n)]
    path.moveTo(pts[0])
    for i in range(n - 1):
        p0 = pts[max(i - 1, 0)]
        p1 = pts[i]
...
def finitePoly(poly):
    ...
    return [line[N.isfinite(line[:, 0]) & N.isfinite(line[:, 1])] for line in poly]
    ...
    def __init__(self, name, **args):
        ...
        self.add(
            setting.LineSet(
                ...
                descr=_("Line styles to plot the contours using"),
    ...
    def __init__(self, name, **args):
        ...
        self.add(
            setting.Int(
                ...
                5,
                minval=2,
                descr=_("Number of sub-levels to plot between each contour"),
                usertext="Levels",
            )
        )
        self.add(
            setting.Bool(
                "hide", True, descr=_("Hide lines"), usertext=_("Hide"), formatting=True
            )
        )
...
class Contour(plotters.GenericPlotter):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.Bool(
                "smoothing",
                False,
                descr=_("Smooth contour lines using Catmull-Rom splines"),
                usertext=_("Smooth lines"),
                formatting=True,
            ),
            7,
        )
        ...
        s.add(
            ContourLines(
                "Lines", descr=_("Contour lines"), usertext=_("Contour lines")
            ),
        ...
        s.add(
            ContourFills(
                "Fills", descr=_("Fill within contours"), usertext=_("Contour fills")
            ),
    ...
    def userdescription(self):
        ...
        if s.scaling == "manual":
            out.append(
                "manual levels (%s)" % (", ".join([str(i) for i in s.manualLevels]))
            )
        else:
    ...
    def getKeyText(self, number):
        ...
        if s.keyLevels:
            ...
            return utils.formatNumber(
                s.levelsOut[number] * cl.scale, cl.format, locale=self.document.locale
            )
        else:
            return ""
    ...
    def drawKeySymbol(self, number, painter, x, y, width, height):
        ...
        painter.setPen(self.settings.Lines.get("lines").makePen(painter, number))
        painter.drawLine(
    ...
    def _plotContours(
        self, painter, posn, axes, linestyles, contours, showlabels, hidelines, clip
    ):
        ...
        for num, linelist in enumerate(contours):
            if showlabels and num < len(s.levelsOut):
                ...
                text = utils.formatNumber(
                    number * cl.scale, cl.format, locale=self.document.locale
                )
                rend = utils.Renderer(
                    painter,
                    font,
            ...
            # vectorize: concatenate all curves, convert once, split back
            allx = N.concatenate([c[:, 0] for c in linelist])
            ally = N.concatenate([c[:, 1] for c in linelist])
            allxplt = axes[0].dataToPlotterCoords(posn, allx)
            allyplt = axes[1].dataToPlotterCoords(posn, ally)

            offset = 0
            for curve in linelist:
                n = len(curve)
                xplt = allxplt[offset : offset + n]
                yplt = allyplt[offset : offset + n]
                offset += n
        ...
        with utils.painter_state(painter):
            ...
            for i in range(linelabeller.getNumPolySets()):
                ...
                for poly in polyset:
                    if smoothing and poly.count() >= 3:
                        painter.drawPath(_catmullRomPath(poly))
                    else:
    ...
    def plotContourFills(self, painter, posn, axes, clip):
        ...
        for num, polylist in enumerate(self._cachedpolygons):
            ...
            # vectorize: concatenate all polys, convert once, split back
            allx = N.concatenate([p[:, 0] for p in polylist])
            ally = N.concatenate([p[:, 1] for p in polylist])
            allxplt = axes[0].dataToPlotterCoords(posn, allx)
            allyplt = axes[1].dataToPlotterCoords(posn, ally)
