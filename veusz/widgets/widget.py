...
class Widget:
    ...
    def iterSiblings(self, cls=None):
        """Iterate over sibling widgets, excluding self.

        If cls is given, only yield siblings that are instances of cls.
        """
        if self.parent is None:
            return
        for c in self.parent.children:
            if c is not self and (cls is None or isinstance(c, cls)):
                yield c
    ...
    def widgetSiblingIndex(self):
        ...
        if self.parent is None:
            ...
            return next(
                i for i, c in enumerate(self.parent.children)
                if c is self)
