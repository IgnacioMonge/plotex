...
class ImageFile(BoxShape):
    ...
    def __init__(self, parent, name=None):
        ...
        self.addAction(widget.Action(
            'restoreimage', self.actionRestoreImage,
    ...
    def actionRestoreImage(self):
        ...
        ops = []
        defaults = {
            'opacity': 100, 'flipHorz': False, 'flipVert': False,
            'greyscale': False, 'cornerRadius': 0,
            'width': [0.3], 'height': [0.3], 'rotate': [0.],
        }
        for name, default in defaults.items():
            setn = s.get(name)
            if setn.val != default:
                ops.append(
                    document.OperationSettingSet(setn, default))
        if ops:
            self.document.applyOperation(
                document.OperationMultiple(ops,
                    descr=_('restore image')))
    ...
    def drawShape(self, painter, rect):
        ...
        image = None
        if (s.filename != '' and s.filename != '{embedded}' and
                os.path.isfile(s.filename)):
        ...
        if not image or image.isNull() or image.width() == 0:
            ...
            if s.greyscale:
                drawimg = drawimg.convertToFormat(
                    qt.QImage.Format.Format_Grayscale8).convertToFormat(
