...
class ColorBar(axis.Axis):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.Line(
                "Border", descr=_("Colorbar border line"), usertext=_("Border")
            ),
    ...
    def _axisDraw(self, posn, parentposn, outerbounds, painter, phelper):
        ...
        if h == "left":
            ...
            delta = (bounds[2] - bounds[0] - totalwidth) / 2.0
            bounds[0] += delta
            bounds[2] -= delta
        elif h == "manual":
        ...
        if v == "top":
            ...
            delta = (bounds[3] - bounds[1] - totalheight) / 2.0
            bounds[1] += delta
            bounds[3] -= delta
        elif v == "manual":
        ...
        if imgwidget is not None:
            ...
            img = utils.makeColorbarImage(
                minval, maxval, axisscale, cmap, trans, direction=s.direction
            )
        else:
            # couldn't find widget
            minval, maxval, axisscale = 0.0, 1.0, "linear"
            img = None
        ...
        minpix, maxpix = self.graphToPlotterCoords(bounds, N.array([minval, maxval]))
        ...
        # actually draw axis
        axis.Axis._axisDraw(self, bounds, parentposn, None, painter, phelper)
