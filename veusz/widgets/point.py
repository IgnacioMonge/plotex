    ...
    def dataDraw(self, painter, axes, posn, cliprect):
        ...
        for xvals, yvals, tvals, ptvals, cvals in (
            ...
            if show_line:
                if interp_type != "linear":
                    self._drawBezierLine(
                        ...
                        xvals, yvals, cliprect, interp_type )
                else:
                    self._drawPlotLine(
                        painter, xplotter, yplotter, posn,
            ...
            if show_markers:
                ...
                if thinfactor <= 1:
                    vpixels = max(
                        abs(cliprect.width()) * abs(cliprect.height()), 1)
                    npts = len(xpltpoint)
                    if npts > vpixels * 4:
                ...
                if thinfactor <= 1:
                    ...
                    xplt, yplt = (
                        xpltpoint[::thinfactor],
                        ypltpoint[::thinfactor])
                ...
                utils.plotMarkers(
                    painter, xplt, yplt, marker, markersize,
