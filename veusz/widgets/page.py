    ...
    def recursivePlotterSearch(self, widget):
        ...
        if widget.isplotter:
            ...
            for axname, depname in widget.affectsAxisRange():
                ...
                can_extend = getattr(widget, 'canExtendFixedAxis', False)
                if resolvedaxis is not None and (
                        resolvedaxis.usesAutoRange() or can_extend):
                    # add dependency for auto-range axes, or widgets
                    # that can extend fixed axes (e.g. brackets)
                    if not resolvedaxis.usesAutoRange() and can_extend:
                        # ensure axis has an entry in ranges
                        if resolvedaxis not in self.ranges:
                            s = resolvedaxis.settings
                            lo = 0 if s.min == 'Auto' else float(s.min)
                            hi = 1 if s.max == 'Auto' else float(s.max)
                            self.ranges[resolvedaxis] = [lo, hi]
