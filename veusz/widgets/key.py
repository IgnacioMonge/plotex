...
class ControlKey:
    ...
    def __init__(self, widget, phelper, parentposn, boxposn, boxdims, textheight):
...
class _GraphControlKey(qt.QGraphicsRectItem, controlgraph._ScaledShape):
    ...
    def __init__(self, parent, params):
        ...
        self.params = params
        self.setScaledRect(
            params.posn[0], params.posn[1], params.dims[0], params.dims[1]
        )
...
class Key(widget.Widget):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.Str("title", "", descr=_("Key title text"), usertext=_("Title")), 0
        )

        s.add(
            setting.KeyBrush(
                "Background", descr=_("Key background fill"), usertext=_("Background")
            ),
            pixmap="settings_bgfill",
        )
        s.add(
            setting.Line("Border", descr=_("Key border line"), usertext=_("Border")),
            pixmap="settings_border",
        )

        s.add(
            setting.AlignHorzWManual(
                "horzPosn",
                "right",
        ...
        while colstats[-1] == 0:
            ...
            while True:
                ...
                newlayout, newcolstats, newrows = self._layoutChunk(
                    entries[victim:], (0, victimcol + 1), (numrows, numcols)
                )
                if newlayout:
    ...
    def _doDrawing(self, painter, phelper, parentposn):
        ...
        if s.title != "":
            ...
            titlefont.setPointSizeF(max(font.pointSize() * 1.2, font.pointSize() + 2))
            titlewidth, titleheight = utils.Renderer(
                painter, titlefont, 0, 0, s.title, doc=self.document
            ).getDimensions()
        ...
        for c in widgets:
            ...
            for i in key_indices:
                ...
                if showtext:
                    w, h = utils.Renderer(
                        painter, font, 0, 0, c.getKeyText(i), doc=self.document
                    ).getDimensions()
        ...
        keyswidth = (maxwidth + height + symbolwidth) * numcols + height * (numcols - 1)
        # total width of box
        ...
        if h == "left":
            ...
            x = parentposn[0] + 0.5 * (parentposn[2] - parentposn[0] - totalwidth)
        elif h == "manual":
        ...
        if v == "top":
            ...
            y = parentposn[1] + 0.5 * (parentposn[3] - parentposn[1] - totalheight)
        elif v == "manual":
            y = (
                parentposn[3]
                - (parentposn[3] - parentposn[1]) * s.vertManual
                - totalheight
            )
        ...
        if s.title:
            ...
            utils.Renderer(
                painter, titlefont, xpos, y, s.title, alignvert=1, doc=self.document
            ).render()
            y += titleheight
        ...
        for plotter, num, xp, yp, lines in layout:
            ...
            with utils.painter_state(painter):
                ...
                plotter.drawKeySymbol(
                    num, painter, sx, ypos + keyoffset, symbolwidth, height
                )
        ...
        phelper.setControlGraph(
            self, [ControlKey(self, phelper, parentposn, boxposn, boxdims, height)]
        )
