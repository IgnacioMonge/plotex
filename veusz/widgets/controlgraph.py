...
def _snap_threshold():
    """Get snap threshold from settings, with fallback."""
    from .. import setting
    return setting.settingdb.get('guides_snap_threshold', 8)

SNAP_THRESHOLD_PX = 8  # fallback default

def _collect_snap_targets(widget, painthelper, exclude_widget=None):
    """Collect snap target coordinates from sibling widgets and graph bounds.

    Returns (vertical_lines, horizontal_lines) in document pixel coords.
    """
    vlines = []  # X positions (vertical guide lines)
    hlines = []  # Y positions (horizontal guide lines)

    if painthelper is None:
        return vlines, hlines

    # graph/parent bounds
    parent = widget.parent if widget else None
    if parent is not None:
        for w, bounds in painthelper.widgetBoundsIterator():
            if w is parent and bounds[0] is not None:
                x0, y0, x2, y2 = bounds
                cx, cy = (x0 + x2) / 2, (y0 + y2) / 2
                vlines.extend([x0, cx, x2])
                hlines.extend([y0, cy, y2])
                break

    # sibling widget bounds
    if parent is not None:
        for child in parent.children:
            if child is widget or child is exclude_widget:
                continue
            key = (child, 0)
            state = painthelper.states.get(key)
            if state is not None and state.bounds[0] is not None:
                b = state.bounds
                vlines.extend([b[0], (b[0]+b[2])/2, b[2]])
                hlines.extend([b[1], (b[1]+b[3])/2, b[3]])

    # user-defined guides (stored on PlotWindow)
    try:
        for win in qt.QApplication.topLevelWidgets():
            tabs = getattr(win, '_tabs', None)
            if tabs:
                for tab in tabs:
                    pw = tab.get('plot')
                    if pw and hasattr(pw, 'guides'):
                        for orient, pos in pw.guides:
                            if orient == 'v':
                                vlines.append(pos)
                            else:
                                hlines.append(pos)
...
def _find_snap(value, targets, threshold):
    """Find the closest snap target within threshold.

    Returns (snapped_value, guide_position) or (value, None).
    """
    ...
    for t in targets:
        ...
        if d < best_dist:
            best_dist = d
            best_target = t
    if best_target is not None and best_dist <= threshold:
        return best_target, best_target
    return value, None


def _snap_position(x, y, widget, cgscale):
    """Snap a position (in document pixel coords) to nearby targets.

    Returns (snapped_x, snapped_y, guide_vx, guide_hy).
    guide_vx/guide_hy are the guide line positions or None.
    """
    # get painthelper from plot window via widget
    painthelper = None
    try:
        doc = widget.document
        # walk up Qt widget hierarchy to find PlotWindow
        for win in doc.signalModified.receivers():
            pass
    except Exception:
        pass

    # try to get painthelper from the widget's document
    # (it's stored on the PlotWindow during rendering)
    if hasattr(widget, '_snap_painthelper'):
        painthelper = widget._snap_painthelper

    from .. import setting
    if not setting.settingdb.get('guides_snap_enabled', True):
        return x, y, None, None
    threshold = _snap_threshold() / cgscale if cgscale > 0 else 5

    vlines, hlines = _collect_snap_targets(widget, painthelper)

    sx, gv = _find_snap(x, vlines, threshold)
    sy, gh = _find_snap(y, hlines, threshold)
    return sx, sy, gv, gh
...
class _ShapeCorner(qt.QGraphicsRectItem, _ScaledShape):
    ...
    def keyPressEvent(self, event):
        """Consume arrow keys to prevent ItemIsMovable movement."""
        event.accept()
...
class _GraphMarginBox(qt.QGraphicsItem):
    ...
    def updateFromLine(self, line, thispos):
        ...
        # snap the proposed center position
        cx = (par.posn[0] + par.posn[2]) / 2 + dx
        cy = (par.posn[1] + par.posn[3]) / 2 + dy
        sx, sy, _, _ = _snap_position(
            cx, cy, par.widget, par.cgscale)
        dx = sx - (par.posn[0] + par.posn[2]) / 2
        dy = sy - (par.posn[1] + par.posn[3]) / 2
...
class _GraphResizableBox(qt.QGraphicsItem):
    ...
    def updateFromLine(self, line, thispos):
        ...
        newx = self.params.posn[0] + dx
        newy = self.params.posn[1] + dy
        sx, sy, _, _ = _snap_position(
            newx, newy, self.params.widget, self.params.cgscale)
        self.params.posn[0] = sx
        self.params.posn[1] = sy
...
class _GraphMovableBox(_GraphMarginBox):
    ...
    def updateFromCorner(self, corner, event):
        if corner == self.cross:
            ...
            # snap the cross position
            sx, sy, _, _ = _snap_position(
                cx, cy, par.widget, par.cgscale)

            dx = sx - (par.deltacrosspos[0] + par.posn[0])
            dy = sy - (par.deltacrosspos[1] + par.posn[1])
...
class _GraphAxisLine(qt.QGraphicsItem):
    ...
    def __init__(self, parent, params):
        ...
        for p in self.pts[0:2]:
            ...
            p.setToolTip(_("Move axis ends"))
        for p in self.pts[2:]:
            p.setCursor(self.curs_zoom[not self.horz])
            p.setToolTip(_("Change axis scale. Double click to reset end."))
        self.line.setCursor( self.curs[self.horz] )
        self.line.setToolTip(_("Move axis position"))
        self.setZValue(2.)
