...
class _Stats:
    ...
    def calculate(self, data, whiskermode):
        ...
        if whiskermode == "min/max":
            ...
            # ``searchsorted`` returns an insertion index; subtracting 1
            # can yield -1 when the upper fence is below the smallest
            # data point (heavily-asymmetric tails). Pre-fix that ``-1``
            # silently wrapped around to the *largest* element via Python
            # negative indexing — the top whisker would then equal the
            # max value, the opposite of what 1.5 IQR is supposed to
            # express. ``max(..., 0)`` matches the symmetric clamp the
            # bottom whisker already uses.
            eltop = max(N.searchsorted(cleaned, self.topquart + 1.5 * iqr) - 1, 0)
        ...
        self.outliers = cleaned[
            (cleaned < self.botwhisker) | (cleaned > self.topwhisker)
        ]
...
class BoxPlot(GenericPlotter):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.ChoiceOrMore(
                ...
                "cb-set1",
                descr=_(
                    "Color palette for boxes (type any colormap name or choose from list)"
                ),
                usertext=_("Fill palette"),
                formatting=True,
            )
        )
        ...
        s.add(
            _BoxFill("Fill", descr=_("Box fill"), usertext=_("Box fill")),
            pixmap="settings_bgfill",
        )
        s.add(
            setting.Line(
                "Border", descr=_("Box border line"), usertext=_("Box border")
            ),
            pixmap="settings_border",
        )
        s.add(
            setting.Line(
                "Whisker", descr=_("Whisker line"), usertext=_("Whisker line")
            ),
        ...
        s.add(
            setting.BoxPlotMarkerFillBrush(
                "MarkersFill",
    ...
    def userdescription(self):
        ...
        s = self.settings
        return "values='%s', position='%s'" % (", ".join(s.values), s.posn)
    ...
    def rangeManual(self):
        ...
        s = self.settings
        ds = []
        for name in ("whiskermin", "whiskermax", "boxmin", "boxmax", "mean", "median"):
    ...
    def plotBox(self, painter, axes, boxposn, posn, width, clip, stats, boxindex=0):
        ...
        if not s.Fill.hide:
            ...
            if s.Fill.transparency > 0:
                color.setAlphaF((100 - s.Fill.transparency) / 100.0)
        ...
        painter.setPen(p)
        swapline(
            painter, boxposn - width / 2, medplt, boxposn + width / 2, medplt, horz
        )
        ...
        if s.showPoints and hasattr(stats, "alldata") and len(stats.alldata) > 0:
            ...
            utils.plotMarkers(painter, px, py, s.pointsMarker, pt_size, clip=clip)
        ...
        if s.showPoints and s.showOutliers and stats.outliers.shape[0] != 0:
            ...
            utils.plotMarkers(painter, x, y, s.outliersmarker, markersize, clip=clip)

        # draw mean
        painter.setPen(s.MarkersLine.makeQPenWHide(painter))
        painter.setBrush(s.MarkersFill.makeQBrushWHide(painter))
        meanplt = axes[not horz].dataToPlotterCoords(posn, N.array([stats.mean]))[0]
        if horz:
            x, y = meanplt, boxposn
        else:
            x, y = boxposn, meanplt
    ...
    def dataDraw(self, painter, axes, widgetposn, clip):
        ...
        if s.calculate:
            ...
            for idx, (vals, plotpos) in enumerate(zip(values, plotposns)):
                ...
                self.plotBox(
                    painter, axes, plotpos, widgetposn, width, clip, stats, boxindex=idx
                )
        else:
