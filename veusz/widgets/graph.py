...
class Graph(widget.Widget):
    ...
    def draw(self, parentposn, painthelper, outerbounds = None):
        ...
        # lazy import cached at module level to avoid repeated lookup
        ...
        # annotations (labels, key) are deferred to draw after axes
        # lazy imports cached at module level to avoid repeated lookup
        from .plotters import FreePlotter
        from .key import Key
        from .bracket import BracketConnector

        deferred_annotations = []
        for c in reversed(self.children):
            # defer annotations to draw them last (on top of axes/grids)
            if isinstance(c, (FreePlotter, Key, BracketConnector)) and not c.isaxis:
                deferred_annotations.append(c)
                continue
        ...
        # draw annotations last so they are on top and selectable
        for c in deferred_annotations:
            c.draw(bounds, painthelper, outerbounds=outerbounds)
