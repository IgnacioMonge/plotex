...
class MainWindow(qt.QMainWindow):
    ...
    def _initConsole(self):
        ...
        self._console = consolewindow.ConsoleWindow(self.document, self)
        self._console.hide()
        self.addDockWidget(qt.Qt.DockWidgetArea.BottomDockWidgetArea, self._console)
    ...
    def _slotTabChanged(self, index):
        ...
        if self._currentTab is not None:
            ...
            if oldidx >= 0:
                ...
                self._tabs[oldidx]["selwidgets"] = list(self.treeedit.selwidgets)
    ...
    def _buildStatusBarZoomControls(self, statusbar):
        ...
        layout.addWidget(
            _toolbtn(
                "kde-zoom-width-veuszedit", _("Zoom to width"), self._delegateZoomWidth
            )
        )
        ...
        # Zoom-out button
        layout.addWidget(
            _toolbtn("kde-zoom-out", _("Zoom out (Ctrl+−)"), self._delegateZoomOut)
        )
        ...
        # Zoom-in button
        layout.addWidget(
            _toolbtn("kde-zoom-in", _("Zoom in (Ctrl++)"), self._delegateZoomIn)
        )
        ...
        layout.addWidget(
            _toolbtn(
                "veusz-view-fullscreen",
                _("Full screen (Ctrl+F11)"),
    ...
    def slotPrintPreview(self):
        ...
        if not self.document.getVisiblePages():
            qt.QMessageBox.warning(self, _("Error - Plotex"), _("No pages to print"))
            return
    ...
    def _createSplit(self, idx, direction, orientation):
        ...
        # focus feedback
        primary.setStyleSheet("PlotWindow { border: %s; }" % self._splitFocusBorder)
        secondary.setStyleSheet("PlotWindow { border: %s; }" % self._splitNoBorder)
    ...
    def _slotCloseTab(self, index):
        ...
        if doc.isModified():
            # ask to save
            name = os.path.basename(state["filename"]) or state.get(
                "untitled_label", _("Untitled")
            )
    ...
    def _getPlotexDropFiles(self, event):
        ...
        if not mime.hasUrls():
            ...
            urls = [u for u in urls if os.path.splitext(u)[1].lower() in supported]
            return urls
    ...
    def _defineMenus(self):
        ...
        self.vzactions = {
            ...
            "edit.copy_as_png": a(
                self,
                _("Copy current page as PNG bitmap (for PowerPoint)"),
                _("Copy as &PNG"),
                self.slotCopyAsPNG,
                icon="kde-edit-copy",
                key="Ctrl+Shift+C",
            ...
            "view.ribbon": a(
                self, _("Show or hide ribbon"), _("Ribbon"), None, checkable=True
            ),
            "view.guides": a(
                self,
                _("Show or hide alignment guides"),
                _("Show guides"),
                self.slotToggleGuides,
                icon="guide_show",
                checkable=True,
            ),
            "view.addHGuide": a(
                self,
                _("Add horizontal guide at page center"),
                _("Add H guide"),
                self.slotAddHGuide,
                icon="guide_addh",
            ),
            "view.addVGuide": a(
                self,
                _("Add vertical guide at page center"),
                _("Add V guide"),
                self.slotAddVGuide,
                icon="guide_addv",
            ),
            "view.clearGuides": a(
                self,
                _("Reset guides to center cross"),
                _("Reset guides"),
                self.slotClearGuides,
                icon="guide_clear",
            ),
            "view.rulers": a(
                self,
                _("Show or hide rulers"),
                _("Rulers"),
                self.slotToggleRulers,
                icon="veusz-ruler",
                checkable=True,
            ),
            "view.importColormap": a(
                self,
                _("Import colormap from file (.txt, .gpl, .cpt)"),
                _("Import colormap…"),
                self.slotImportColormap,
            ),
            "view.resetlayout": a(
                self,
                _("Reset dock panels to default layout"),
                _("Reset Window Layout"),
                self.slotResetLayout,
            ),
            "view.printpreview": a(
                self,
                _("Preview the document before printing"),
                _("Print preview"),
                self.slotPrintPreview,
                icon="veusz-print-preview",
            ),
            "view.splitH": a(
                self,
                _("Split view horizontally (top/bottom)"),
                _("Horizontal"),
                self.slotToggleSplitH,
                icon="veusz-split-horizontal",
                checkable=True,
            ),
            "view.splitV": a(
                self,
                _("Split view vertically (left/right)"),
                _("Vertical"),
                self.slotToggleSplitV,
                icon="veusz-split-vertical",
                checkable=True,
            ),
            "data.import": a(
                self,
                _("Import data into Plotex"),
                _("&Import…"),
                self.slotDataImport,
            ...
            "help.home": a(
                self,
                _("Go to the Plotex home page on the internet"),
                _("Home page"),
            ...
            "help.tutorial": a(
                self,
                _("An interactive Plotex tutorial"),
                _("Tutorial"),
            ...
            "help.examples": a(
                self,
                _("Open example documents"),
                _("Examples"),
                self.slotHelpExamples,
                icon="kde-document-open",
            ),
            "edit.commandpalette": a(
                self,
                _("Search and run any command"),
                _("Command Palette"),
                self.slotCommandPalette,
                key="Ctrl+K",
            ),
        }
    ...
    def _populateRibbon(self):
        ...
        if hasattr(openBtn, "setMenu"):
            ...
            openBtn.setPopupMode(qt.QToolButton.ToolButtonPopupMode.MenuButtonPopup)
        ...
        g.addSmallColumn([acts["add.bar"], acts["add.histo"], acts["add.function"]])
        g.addSmallColumn([acts["add.boxplot"], acts["add.fit"], acts["add.violin"]])
        ...
        g.addSmallColumn(
            [acts["file.print"], acts["edit.copy_as_image"], acts["edit.copy_as_png"]]
        )
        ...
        g.addSmallColumn([acts["edit.delete"], acts["edit.rename"], acts["edit.show"]])
        g.addSmallColumn(
            [acts["edit.moveup"], acts["edit.movedown"], acts["edit.hide"]]
        )
        ...
        g.addSmallColumn([acts["add.page"], acts["add.grid"], acts["add.graph"]])

        g = ins.addGroup(_("Plots"))
        g.addLargeButton(acts["add.xy"])
        g.addSmallColumn([acts["add.bar"], acts["add.histo"], acts["add.function"]])
        g.addSmallColumn([acts["add.boxplot"], acts["add.violin"], acts["add.fit"]])
        g.addSmallColumn([acts["add.piechart"], acts["add.heatmap"], acts["add.image"]])

        g = ins.addGroup(_("Statistics"))
        g.addSmallColumn(
            [acts["add.pareto"], acts["add.qqplot"], acts["add.ridgeline"]]
        )

        g = ins.addGroup(_("Clinical"))
        g.addSmallColumn(
            [acts["add.blandaltman"], acts["add.kaplanmeier"], acts["add.roccurve"]]
        )
        g.addSmallColumn(
            [acts["add.polartrending"], acts["add.covariance"], acts["add.contour"]]
        )

        g = ins.addGroup(_("More"))
        g.addSmallColumn(
            [acts["add.polar"], acts["add.ternary"], acts["add.vectorfield"]]
        )
        g.addSmallColumn(
            [acts["add.scene3d"], acts["add.graph3d"], acts["add.nonorthpoint"]]
        )

        g = ins.addGroup(_("Annotations"))
        g.addSmallColumn([acts["add.key"], acts["add.label"], acts["add.colorbar"]])
        g.addSmallColumn(
            [acts["add.bracket"], acts["add.axismenu"], acts["add.shapemenu"]]
        )
        ...
        g.addSmallColumn([acts["view.graphrecenter"], acts["view.graphreset"]])
        ...
        g.addSmallColumn(
            [acts["view.guides"], acts["view.addHGuide"], acts["view.addVGuide"]]
        )
        g.addSmallColumn([acts["view.clearGuides"]])

        g = view.addGroup(_("Panels"))
        g.addLargeButton(acts["view.console"])
        g.addSmallColumn([acts["view.datanav"], acts["view.resetlayout"]])
        ...
        g.addSmallColumn([acts["help.home"], acts["help.bug"], acts["help.update"]])
    ...
    def defineViewWindowMenu(self):
        ...
        self.menus["view"].aboutToShow.connect(self.slotAboutToShowViewWindow)
    ...
    def slotHelpBug(self):
        """Open the Plotex issue tracker on GitHub."""
        qt.QDesktopServices.openUrl(
            qt.QUrl("https://github.com/IgnacioMonge/plotex/issues")
        )
    ...
    def askTutorial(self):
        ...
        retn = qt.QMessageBox.question(
            ...
            _("Plotex Tutorial"),
            _(
                "Plotex includes a tutorial to help get you started.\n"
    ...
    def askVersionCheck(self, mininterval=2):
        ...
        retn = qt.QMessageBox.question(
            ...
            _("Version check"),
            _(
                "Plotex will periodically check for new versions and\n"
    ...
    def askFeedbackCheck(self, mininterval=3):
        ...
        retn = qt.QMessageBox.question(
            ...
            _(
                "Plotex can automatically send anonymous feedback "
    ...
    def slotNewVersion(self, ver):
        ...
        self.vzactions["help.update"].setText(_("Download new Plotex %s") % ver)
    ...
    def closeEvent(self, event):
        ...
        for i, state in enumerate(self._tabs):
            ...
            if doc.isModified() and not discard_all:
                ...
                msgbox.setDefaultButton(qt.QMessageBox.StandardButton.Save)
                # count remaining unsaved tabs
                unsaved = sum(1 for s in self._tabs[i:] if s["document"].isModified())
                if unsaved > 1:
        ...
        setdb["geometry_mainwindowstate_b64"] = base64.b64encode(bytes(data)).decode(
            "ascii"
        )
    ...
    def setupWindowGeometry(self):
        ...
        if "geometry_mainwindowstate_b64" in setdb:
            try:
                state_data = base64.b64decode(setdb["geometry_mainwindowstate_b64"])
            except Exception:
                pass
    ...
    def slotResetLayout(self):
        ...
        for dock in [self.treeedit, self.propdock, self.formatdock, self.datadock]:
            dock.show()
        self._console.hide()
    ...
    def updateTitlebar(self):
        ...
        if self.filename == "":
            ...
            self.setWindowTitle(_("%s - Plotex") % os.path.basename(self.filename))
    ...
    def openFile(self, filename):
        ...
        for i, state in enumerate(self._tabs):
            if state["filename"] and os.path.abspath(state["filename"]) == absname:
                self._tabwidget.setCurrentIndex(i)
                return
        ...
        try:
            ...
            def _onprogress(current, total, phase_msg):
                if phase_msg is not None:
                    loadlabel.setText("%s \u2014 %s" % (basename, phase_msg))
            ...
            if e.backtrace:
                ...
                qt.QMessageBox.critical(
                    self, _("Error opening %s - Plotex") % filename, str(e)
                )
            return False
        finally:
    ...
    def slotFileOpen(self):
        ...
        filename = self.fileOpenDialog(
            [_("Plotex document files (%s)") % " ".join(filters)], _("Open")
        )
        if filename:
            self.openFile(filename)
