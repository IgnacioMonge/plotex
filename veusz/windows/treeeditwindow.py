...
class PropertyList(qt.QWidget):
    ...
    def __init__(self, document, showformatsettings=True,
        ...
        self.layout.setSpacing( max(self.layout.spacing()*3//4, 4) )
        self.layout.setContentsMargins(4,4,4,4)

        # search/filter bar at the top of the property panel
        self.searchEdit = qt.QLineEdit()
        self.searchEdit.setPlaceholderText(_("Filter settings..."))
        self.searchEdit.setClearButtonEnabled(True)
        self.layout.addWidget(self.searchEdit, 0, 0, 1, -1)
        self.searchEdit.textChanged.connect(self._filterSettings)

        self.childlist = []
        self.setncntrls = {}     # map setting name to controls

        # update reset button visibility when document changes
    def getConsole(self):
    ...
    def _filterSettings(self, text):
        """Filter visible settings based on search text."""
        text = text.strip().lower()
        for name, ctrls in self.setncntrls.items():
            lab = ctrls[0]
            # get the label text for matching
            labtext = name.lower()
            if hasattr(lab, 'labelicon'):
                labtext = lab.labelicon.text().lower()
            visible = (not text) or (text in labtext) or (text in name.lower())
            # show/hide label (col 0) and control (col 1)
            ctrls[0].setVisible(visible)
            ctrls[1].setVisible(visible)
...
class FormatDock(qt.QDockWidget):
    ...
    def __init__(self, document, treeedit, *args):
        ...
        self.setWindowTitle(_("Formatting - Plotex"))
        self.setObjectName("veuszformattingdock")
        self.setMinimumWidth(260)

        self.document = document
        self.tabwidget = None
        self._lastWidgetIds = None
    ...
    def selectedWidgets(self, widgets, setnsproxy):
        ...
        # skip full rebuild if exactly the same widgets are selected
        wids = tuple(id(w) for w in widgets) if widgets else ()
        if wids == self._lastWidgetIds and self.tabwidget is not None:
            return
        self._lastWidgetIds = wids
...
class PropertiesDock(qt.QDockWidget):
    ...
    def __init__(self, document, treeedit, *args):
        qt.QDockWidget.__init__(self, *args)
        self.setWindowTitle(_("Properties - Plotex"))
        self.setObjectName("veuszpropertiesdock")
        self.setMinimumWidth(260)

        self.document = document
        self._lastSetnsproxy = None
    ...
    def slotWidgetsSelected(self, widgets, setnsproxy):
        ...
        if setnsproxy is self._lastSetnsproxy:
            return
        self._lastSetnsproxy = setnsproxy
...
class TreeEditDock(qt.QDockWidget):
    ...
    def __init__(self, document, parentwin):
        ...
        self.setWindowTitle(_("Editing - Plotex"))
        self.setObjectName("veuszeditingwindow")
        self.setMinimumWidth(200)
        self.selwidgets = []
        self._enableButtonsCache = None
        self._enableButtonsCount = 0
        ...
        # receive change in selection (debounced)
        self._selectionTimer = qt.QTimer(self)
        self._selectionTimer.setSingleShot(True)
        self._selectionTimer.setInterval(15)
        self._selectionTimer.timeout.connect(self._processSelection)
    ...
    def _applyThemeToWidget(self, action):
        """Apply a plot theme to the selected widget only."""
        if not self.selwidgets:
            return
        from ..utils.plotthemes import THEMES
        key = action.data()
        ...
        w = self.selwidgets[0]
        self.document.applyOperation(
            document.OperationApplyThemeToWidget(w, theme['settings']))

    def slotDocumentWiped(self):
        """If the document is wiped, reselect root widget."""
        self.selectWidget(self.document.basewidget)

    def setDocument(self, newdocument, newtreemodel):
        """Switch to a different document (for tab switching)."""
        # disconnect old signals
        try:
            self.document.sigWiped.disconnect(self.slotDocumentWiped)
        ...
        self.treemodel = newtreemodel
        self.treeview.setModel(newtreemodel)

        # reconnect
        newdocument.sigWiped.connect(self.slotDocumentWiped)
        self.treeview.selectionModel().selectionChanged.connect(
            self._onSelectionChanged)
    ...
    def _onSelectionChanged(self, current, previous):
        """Selection changed — restart debounce timer."""
        self._selectionTimer.start()

    def _processSelection(self):
        """Process the debounced selection change."""
    ...
    def contextMenuEvent(self, event):
        ...
        if self.selwidgets:
            # Remove previous dynamic style actions
            for act in getattr(self, '_styleActions', []):
                self.contextmenu.removeAction(act)
            self._styleActions = []

            # Add style submenu for container widgets
            w = self.selwidgets[0]
            _containers = {'graph', 'page', 'grid', 'polar', 'ternary',
                           'scene3d', 'graph3d'}
            if len(self.selwidgets) == 1 and w.typename in _containers:
                from ..utils.plotthemes import getThemeNames, THEMES
                sep = self.contextmenu.addSeparator()
                self._styleActions.append(sep)

                submenu = qt.QMenu(_('Apply style'), self.contextmenu)
                for key, name, descr in getThemeNames():
                    act = submenu.addAction(name)
                    act.setToolTip(descr)
                    act.setData(key)
                submenu.triggered.connect(self._applyThemeToWidget)
                menuAction = self.contextmenu.addMenu(submenu)
                self._styleActions.append(menuAction)
    ...
    def _checkPageChange(self):
        ...
        if w is not None:
            ...
            for i, c in enumerate(self.document.basewidget.children):
                if c is w:
    ...
    def _enableCorrectButtons(self):
        ...
        # build a cache key from selected widget type + parent chain
        cachekey = None
        if selw is not None:
            chain = []
            w = selw
            while w is not None:
                chain.append(w.typename)
                w = w.parent
            cachekey = tuple(chain)

        if (hasattr(self, '_enableButtonsCache') and
                cachekey == self._enableButtonsCache and
                len(self.selwidgets) == self._enableButtonsCount):
            return
        self._enableButtonsCache = cachekey
        self._enableButtonsCount = len(self.selwidgets)
    ...
    def _constructToolbarMenu(self):
        ...
        for widgettype in (
                ...
                'xy', 'bar', 'histo', 'fit', 'function', 'boxplot', 'violin',
                'piechart', 'heatmap',
                'pareto', 'qqplot', 'ridgeline',
                'blandaltman', 'kaplanmeier', 'roccurve', 'polartrending',
                'bracket',
            ...
            # short label for ribbon (no "Add" prefix)
            shortname = {
                'xy': 'XY', 'histo': 'Histogram', 'fit': 'Fit',
                'boxplot': 'Box plot', 'violin': 'Violin',
                'piechart': 'Pie chart', 'heatmap': 'Heatmap',
                'blandaltman': 'Bland-Altman', 'kaplanmeier': 'Kaplan-Meier',
                'roccurve': 'ROC curve', 'polartrending': 'Polar trending',
                'pareto': 'Pareto', 'qqplot': 'Q-Q Plot',
                'ridgeline': 'Ridgeline',
                'bracket': 'Bracket',
                'vectorfield': 'Vector field', 'colorbar': 'Color bar',
                'nonorthpoint': 'Non-orth XY',
                'nonorthfunc': 'Non-orth fn',
                'scene3d': 'Scene 3D', 'graph3d': 'Graph 3D',
                'function3d': 'Fn 3D', 'point3d': 'Point 3D',
                'axis3d': 'Axis 3D', 'surface3d': 'Surface 3D',
                'volume3d': 'Volume 3D', 'imagefile': 'Image file',
                'svgfile': 'SVG file', 'axis-broken': 'Broken axis',
                'axis-function': 'Fn axis',
            }.get(widgettype, widgettype.capitalize())
            actions[actionname] = utils.makeAction(
                self,
                wc.description, shortname,
                slot,
        ...
        actions.update({
            ...
            'edit.duplicate': a(
                self, _('Duplicate selected widget'), _('D&uplicate'),
                self.slotWidgetDuplicate,
                icon='kde-edit-copy', key='Ctrl+D'),
            'edit.delete': a(
                self, _('Remove selected widgets'), _('&Delete'),
                self.slotWidgetDelete,
                icon='kde-edit-delete', key='Delete'),
            'edit.rename': a(
        ...
        insertactions = (
            ['insert.layout', _('Layout'), [
                'add.page', 'add.grid', 'add.graph',
            ]],
            '',
            ['insert.2d', _('2D Plots'), [
                'add.xy', 'add.function', 'add.fit',
                'add.covariance', 'add.image', 'add.contour',
                'add.vectorfield',
            ]],
            ['insert.stat', _('Statistical'), [
                'add.bar', 'add.histo', 'add.boxplot',
                'add.violin', 'add.piechart', 'add.heatmap',
            ]],
            ['insert.clinical', _('Clinical / Scientific'), [
                'add.blandaltman', 'add.kaplanmeier',
                'add.roccurve', 'add.polartrending',
            ]],
            ['insert.special', _('Specialized'), [
                'add.polar', 'add.ternary',
                'add.nonorthpoint', 'add.nonorthfunc',
            ]],
            '',
            ['insert.3d', _('3D'), [
                'add.scene3d', 'add.graph3d', 'add.axis3d',
                'add.function3d', 'add.point3d',
                'add.surface3d', 'add.volume3d',
            ]],
            '',
            ['insert.annot', _('Annotations'), [
                'add.key', 'add.label', 'add.colorbar',
                'add.bracket', 'add.axismenu', 'add.shapemenu',
            ]],
        )
        menuitems = (
    ...
    def slotWidgetDuplicate(self):
        """Duplicate selected widget(s) in-place."""
        if not self.selwidgets:
            return
        ...
        mimetext = document.getWidgetMime(mimedata)
        if mimetext and document.isWidgetMimePastable(parent, mimetext):
            op = document.OperationWidgetPaste(parent, mimetext)
    ...
    def slotWidgetCopyAsImage(self):
        """Copy current page to the clipboard as a single image format."""
        mime_map = {
            'svg': ['image/svg+xml'],
            'png': ['image/png', 'PNG'],
            'bmp': ['image/x-bmp'],
        ...
        fmt = setdb.get('copyimage_format', 'png')
        mimes = mime_map.get(fmt, mime_map['png'])

        def checkDone(export):
            ...
            try:
                ...
                clipboard = qt.QApplication.clipboard()
                data = qt.QMimeData()
                fname = os.path.join(tmpdir.name, 'tmp.{}'.format(fmt))
                ...
                qt.QMessageBox.critical(self, _("Error - Plotex"), _("Error while exporting image"))
            self.checktimer.stop()

        # use copy-image specific DPI/quality
        export = document.AsyncExport(
            self.document,
            bitmapdpi=setdb.get('copyimage_dpi', 150),
            pdfdpi=setdb['export_DPI_PDF2'],
            antialias=setdb['export_antialias'],
            color=setdb['export_color'],
            quality=setdb.get('copyimage_quality', 90),
        ...
        self.checktimer.timeout.connect(lambda: checkDone(export))
    ...
    def slotWidgetDelete(self):
        ...
        # select parent or sibling in the same graph/page
        parent = widgets[0].parent if widgets[0].parent else self.document.basewidget
        # try to select a sibling at the same position
        if parent.children:
            idx = min(minindex, len(widgetlist) - 1)
            # find a widget that shares the same parent
            nextwidget = parent
    ...
    def updateSelectMenu(self):
        ...
        # get graph container for "this graph" scope
        graph = widget.parent
        graphlabel = graph.name if graph else ''

        # -- This graph --
        menu.addAction(
            _("'%s' siblings in this graph (%s)") % (wtype, graphlabel),
        ...
        if page and page is not widget:
            menu.addAction(
                _("All '%s' on page '%s'") % (wtype, page.name),
                lambda: self._selectWidgetsTypeAndOrName(
                    wtype, None, root=page))
            menu.addAction(
                _("All '%s' called '%s' on page '%s'") % (
                    wtype, name, page.name),
                lambda: self._selectWidgetsTypeAndOrName(
                    wtype, name, root=page))

            menu.addSeparator()

        # -- Entire document --
        menu.addAction(
            _("All '%s' in document") % wtype,
            lambda: self._selectWidgetsTypeAndOrName(wtype, None))
        menu.addAction(
            _("All '%s' called '%s' in document") % (wtype, name),
            lambda: self._selectWidgetsTypeAndOrName(wtype, name))

class _CopyToDialog(qt.QDialog):
    """Dialog with search filter and checkable list for selecting
    target widgets when copying a setting."""

    def __init__(self, widgetpaths, parent=None):
        super().__init__(parent)
        self.setWindowTitle(_('Copy setting to widgets'))
        self.resize(500, 400)
        ...
        # search box
        self.searchEdit = qt.QLineEdit()
        self.searchEdit.setPlaceholderText(_('Filter…'))
        self.searchEdit.setClearButtonEnabled(True)
        self.searchEdit.textChanged.connect(self._filterList)
        layout.addWidget(self.searchEdit)

        # select all / none
        btnrow = qt.QHBoxLayout()
        btnAll = qt.QPushButton(_('Select all'))
        btnAll.clicked.connect(self._selectAll)
        btnNone = qt.QPushButton(_('Select none'))
        btnNone.clicked.connect(self._selectNone)
...
class SettingLabel(qt.QWidget):
    ...
    def __init__(self, document, setting, setnsproxy):
        ...
        self._lastDefault = self.setting.isDefault()
        self._lastRef = self.setting.isReference()
    ...
    def slotDocModified(self, ismodified):
        ...
        # skip expensive updates if nothing relevant changed
        curDefault = self.setting.isDefault()
        curRef = self.setting.isReference()
        changed = (curDefault != self._lastDefault or
                   curRef != self._lastRef)
        self._lastDefault = curDefault
        self._lastRef = curRef
        ...
        f = self.labelicon.font()
        multivalued = self.setnsproxy.multivalued(self.setting.name)
        wantBold = (not curDefault) or multivalued
        wantItalic = multivalued
        if f.bold() != wantBold or f.italic() != wantItalic:
            f = qt.QFont(f)
            f.setBold(wantBold)
            f.setItalic(wantItalic)
    ...
    def addCopyToWidgets(self, menu):
        """Add a 'Select widgets…' action that opens a searchable dialog."""

        def getWidgetsOfType(widget, widgettype, result):
            """Recursively build up a list of widgets of the type given."""
            for w in widget.children:
                if w.typename == widgettype:
                    result.append(w)
                getWidgetsOfType(w, widgettype, result)
        ...
        widgetlist = []
        getWidgetsOfType(
            self.document.basewidget, setwidget.typename, widgetlist)
        widgetpaths = sorted(
            w.path for w in widgetlist if w != setwidget)

        if not widgetpaths:
            return

        setpath = self.setting.path
        wpath = setwidget.path
        setpath = setpath[len(wpath):]

        def openSelector():
            dlg = _CopyToDialog(widgetpaths, self)
            if dlg.exec() == qt.QDialog.DialogCode.Accepted:
                selected = dlg.selectedWidgets()
                if selected:
                    ops = []
                    for wp in selected:
                        ops.append(document.OperationSettingSet(
                            wp + setpath, self.setting.get()))
                    self.document.applyOperation(
                        document.OperationMultiple(ops,
                            descr=_('copy setting')))

        action = menu.addAction(_('Select widgets…'))
        action.triggered.connect(openSelector)
    ...
    def settingMenu(self, pos):
        ...
        if self.setting.path[:12] != '/StyleSheet/':
            ...
            # scope: this graph
            parent = self._clickwidget.parent
            parentlabel = parent.name if parent else ''
            copyto.addAction(
                _("'%s' siblings in this graph (%s)")
                % (wtype, parentlabel),
            ...
            # scope: this page
            page = self._clickwidget
            while page is not None and page.typename != 'page':
                page = page.parent
            if page and page is not self._clickwidget:
                copyto.addAction(
                    _("All '%s' called '%s' on this page (%s)")
                    % (wtype, name, page.name),
                    lambda checked=False, p=page: self.actionCopyNamedToPage(p))
                copyto.addAction(
                    _("All '%s' on this page (%s)")
                    % (wtype, page.name),
                    lambda checked=False, p=page: self.actionCopyToPage(p))
                copyto.addSeparator()
            ...
            copyto.addAction(
                _("All '%s' called '%s' in document")
                % (wtype, name),
                self.actionCopyTypedNamedWidgets)

            copyto.addSeparator()

            # scope: pick specific widgets
    ...
    def actionCopyNamedToPage(self, page):
        """Copy setting to widgets with same name and type on this page."""
        self.document.applyOperation(
            document.OperationSettingPropagate(
                self.setting,
                widgetname=self._clickwidget.name,
                root=page) )

    def actionCopyToPage(self, page):
        """Copy setting to all widgets of same type on this page."""
        self.document.applyOperation(
            document.OperationSettingPropagate(
                self.setting,
                root=page) )
