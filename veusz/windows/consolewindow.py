...
class _ConsoleCompleter(qt.QCompleter):
    """Completer for the console command line.

    Provides completions from:
    - Plotex commands (Add, Set, Get, etc.)
    - numpy functions
    ...
    def __init__(self, parent=None):
        qt.QCompleter.__init__(self, [], parent)
        self.setCaseSensitivity(qt.Qt.CaseSensitivity.CaseSensitive)
        self.setCompletionMode(
            qt.QCompleter.CompletionMode.PopupCompletion)
        self.setFilterMode(qt.Qt.MatchFlag.MatchStartsWith)
        self._words = []

    def updateCompletions(self, interpreter):
        """Rebuild completion list from interpreter context."""
        words = set()

        # commands and globals from the interpreter
        if hasattr(interpreter, 'globals'):
            for name in interpreter.globals:
                if not name.startswith('_'):
                    words.add(name)

        # dataset names from document
        if hasattr(interpreter, 'document'):
            for name in interpreter.document.data:
                words.add(name)

        self._words = sorted(words)
        self.setModel(qt.QStringListModel(self._words))
...
class _CommandEdit(qt.QLineEdit):
    ...
    def __init__(self, *args):
        ...
        self.setToolTip(
            _("Python console. Tab for completion, "
              "Up/Down for history"))

        # set up completer
        self.consolecompleter = _ConsoleCompleter(self)
        self.setCompleter(self.consolecompleter)
    ...
    def keyPressEvent(self, key):
        """ Overridden to handle history and tab completion. """

        code = key.key()

        # Tab: trigger completion on the current word fragment
        if code == qt.Qt.Key.Key_Tab:
            text = self.text()
            ...
            if prefix:
                self.consolecompleter.setCompletionPrefix(prefix)
                if self.consolecompleter.completionCount() == 1:
                    # single match: insert directly
                    completion = self.consolecompleter.currentCompletion()
                    newtext = text[:start] + completion + text[cursor:]
                    self.setText(newtext)
                    self.setCursorPosition(start + len(completion))
                else:
                    # multiple matches: show popup
                    self.consolecompleter.complete()
            return
...
introtext=_('''Welcome to <b><font color="#a000a0">Plotex %s</font></b> --- a scientific plotting application.<br>
Fork of Veusz by Jeremy Sanders. Author: M. Ignacio Monge Garc\u00eda.<br>
Plotex comes with ABSOLUTELY NO WARRANTY. Plotex is Free Software, and you are<br>
...
class ConsoleWindow(qt.QDockWidget):
    ...
    def __init__(self, thedocument, *args):
        ...
        self.setWindowTitle(_("Console - Plotex"))
        ...
        # initialise autocompletion from interpreter context
    ...
    def slotEnter(self, command):
        ...
        if comp is None or (
            ...
            # modify the prompt
            self._prompt.setText('>>>')
            # refresh completions (new datasets/variables may exist)
            self._updateCompleter()

    def _updateCompleter(self):
        """Refresh autocompletion word list from interpreter."""
        self._inputedit.consolecompleter.updateCompletions(
            self.interpreter)
