...
class StepIntro(TutorialStep):
    def __init__(self, mainwin):
        TutorialStep.__init__(
            self, _('''
<h1>Welcome to Plotex!</h1>

<p>This tutorial aims to get you working with Plotex as quickly as
possible.</p>
...
<p>Plots in Plotex are constructed from <i>widgets</i>.  Different
...
Plotex supports LaTeX style formatting for labels, so you could include
...
<p>(Plotex uses Python syntax for its functions, so the power operator
...
class FunctionLineFormatting(TutorialStep):
    def __init__(self, mainwin):
        TutorialStep.__init__(
            self, _('''
<p>Plotex lets you choose a line style, thickness and color for the
function line.</p>
...
<p>Many widgets in Plotex plot datasets. Datasets can be imported from
...
class DataImportDialog2(TutorialStep):
    def __init__(self, mainwin):
        TutorialStep.__init__(
            self, _('''
<p>Plotex will try to guess the datatype - numeric, text or date - from
the data in the file or you can specify it manually.</p>

<p>Several different data formats are supported in Plotex and plugins
...
class DataImportDialog3(TutorialStep):
    def __init__(self, mainwin):
        TutorialStep.__init__(
            self, _('''
<p>Notice how Plotex has loaded the three different datasets from the
...
<p>The point plotting widget plots datasets loaded in Plotex.</p>
...
class SetXYLine(TutorialStep):
    def __init__(self, mainwin):
        TutorialStep.__init__(
            self, _('''
<p>Plotex has now plotted the data on the graph. You can manipulate how
...
<p>Plotex documents are simple text files which can be easily modified
...
<p>Thank you for working through this Plotex tutorial. We hope you
enjoy using Plotex!</p>
...
class TutorialDock(qt.QDockWidget):
    ...
    def __init__(self, document, mainwin, *args):
        ...
        self.setWindowTitle('Tutorial - Plotex')
